"""
TANI V12.4 FINAL CONNECTED - BOT 1 Entry + Baca DNA
- Kalo .dna_tani_v12.json ADA: pakai voting 40 petani (garang)
- Kalo BELUM ADA: pakai EMA simpel (aman, ga fail)
File wajib: tani.py
"""
...
CONFIG = {
    ...
    "QUORUM_FILE": ".quorum_rekomendasi.json",
...
class PosMan:
    ...
    def open(self,kep,entry,sl,tp1,tp2,tp3,sl_dist,lot,atr):
        ...
        save_json(self.path,self.pos); send_tg(f"🔥 <b>OPEN {kep}</b> {entry:.2f} SL {sl:.2f} TP1 {tp1:.2f}")
    def manage(self,price,atr):
        ...
        if kep=="BUY":
            ...
            if price<=p["sl"]: send_tg(f"❌ <b>SL HIT</b> {kep}"); self.pos=None; save_json(self.path,{"open":False}); return True
...
def get_signal_with_dna(df):
    """Kalo DNA ada, voting 40 petani. Kalo ga ada, EMA simpel"""
    dna=load_json(CONFIG["DNA_FILE"], None)
    quorum_cfg=load_json(CONFIG["QUORUM_FILE"], {})
    qk=quorum_cfg.get("qk", CONFIG["DEFAULT_QK"])
    qr=quorum_cfg.get("qr", CONFIG["DEFAULT_QR"])

    if dna is None or len(dna)<10:
        # MODE SIMPEL (belum ada DNA)
        print("DNA belum ada, pakai EMA simpel")
        e20=df['Close'].ewm(20).mean().iloc[-1]
        e50=df['Close'].ewm(50).mean().iloc[-1]
        kep="BUY" if e20>e50 else "SELL"
        conf=0.68
        return kep, conf, False, qk

    # MODE DNA GARANG - voting 40 petani
    print(f"DNA ADA {len(dna)} petani, QUORUM {qk}/{qr}")
    votes=[]
    for name,cfg in dna.items():
        # tiap petani punya ema_fast/slow beda
        fast=cfg.get('ema_fast',20)
        slow=cfg.get('ema_slow',50)
        if slow<=fast: slow=fast+20
        e_f=df['Close'].ewm(fast).mean().iloc[-1]
        e_s=df['Close'].ewm(slow).mean().iloc[-1]
        vote=1 if e_f>e_s else -1
        # weight by skor
        weight=max(0.1, cfg.get('skor',1)/10)
        votes.append(vote*weight)

    score=sum(votes)
    total_weight=sum(abs(v) for v in votes) or 1
    prob = (score/total_weight +1)/2 # 0-1

    if prob>0.60:
        kep="BUY"; conf=prob
    elif prob<0.40:
        kep="SELL"; conf=1-prob
    else:
        kep="HOLD"; conf=0.5
    ...
    quorum_pct= max(buy_votes, sell_votes)/len(votes)*100

    print(f"Voting: BUY {buy_votes} SELL {sell_votes} QUORUM {quorum_pct:.1f}% prob {prob:.2f}")

    if quorum_pct < qk:
        print(f"QUORUM kecil {quorum_pct:.1f}% < {qk} skip")
        return "HOLD", conf, True, qk

    return kep, conf, True, qk

def main():
    print(f"=== TANI V12.4 CONNECTED {datetime.now(timezone.utc)} ===")
    fetcher=Fetcher(); posman=PosMan(CONFIG["POSITION_FILE"])
    m5=fetcher.get_klines("5m",300)
    if m5.empty: print("Fail fetch"); return
    ...
    if posman.pos and posman.pos.get("open"):
        if not posman.manage(price,atr):
            print("Posisi aktif, skip entry baru")
            return

    if atr < CONFIG["ATR_MIN"]: print("ATR kecil"); return

    kep, conf, dna_mode, qk = get_signal_with_dna(m5)

    if kep=="HOLD":
        print("HOLD, no signal"); return

    last=load_json(CONFIG["LAST_FILE"],{})
    if time.time()-last.get("time",0) < 1800:
        print("Cooldown 30m"); return
    ...
    save_json(CONFIG["LAST_FILE"],{"keputusan":kep,"time":time.time(),"price":price,"conf":conf,"dna_mode":dna_mode})
    mode_txt="DNA 40 petani" if dna_mode else "EMA simpel"
    send_tg(f"🔥👑 <b>V12.4 {kep}</b> {price:.2f} SL {sl:.2f} TP3 {tp3:.2f}\nConf {conf:.2f} Mode: {mode_txt} ATR {atr:.2f}")
