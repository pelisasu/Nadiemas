"""
🌾👑 TANI V8.8.2 FINAL - WEEKEND EVALUASI + SINYAL TELEGRAM ONLY (TANPA MT5 AUTO)
- Weekend: Sabtu-Minggu OFF, cuma evaluasi + cek harga PAXG
- Weekday: Sinyal Telegram PANEN KECIL / RAYA
- Tanpa MT5 auto trade - GitHub ubuntu-latest only, stabil!
- Cache FIX: key static biar DNA gak reset
"""
...
💰 {kondisi} ATR {atr:.2f}$ RR 1:{tp3_dist/sl_dist:.1f}
ENTRY {entry:.2f} SL {sl:.2f} (-{sl_dist:.1f}$) TP1 {tp1:.2f} TP2 {tp2:.2f} TP3 {tp3:.2f} Lot {lot}
🏚️ GUDANG {bar} {gudang}$ Evo {memory.get('evolutions',0)}x
🧬 TOP {top_str}
✅ SINYAL TELEGRAM ONLY - TANPA MT5 AUTO"""
        ...
        print(f"Foto terkirim {jenis} {keputusan}")
...
def send_weekend_check(price_paxg, price_mt5_est, ofi, memory, dna, offset_history):
    ...
    try:
        ...
        caption=f"🌴 V8.8.2 WEEKEND EVALUASI\n📊 Gudang {gudang}$ Kecil {kecil}x Raya {raya}x Evo {evo}x\n🧬 TOP {top_str}\n💰 PAXG {price_paxg:.2f} MT5 {price_mt5_est:.2f} OFI {ofi:+.2f} Offset avg {avg_offset:.2f}\n🕐 Sabtu 04:00 - Senin 05:00 WIB OFF\n⏰ Auto ON Senin 05:00 WIB!"
        requests.post(f"https://api.telegram.org/bot{token}/sendPhoto",json={"chat_id":chat,"photo":"https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=800","caption":caption},timeout=15)
...
def ratu_tani_v8():
    print(f"=== 🌾👑 RATU TANI V8.8.2 TANPA MT5 {datetime.now()} ===")
    ...
    if is_off:
        ...
        if time.time()-last_weekend>21600:
            ...
            print(f"Weekend cooldown { (time.time()-last_weekend)/3600:.1f}j")
            save_json(CONFIG["LAST_FILE"], last)
        print(f"💾 File disimpan: DNA {len(dna)} petani, Gudang {memory.get('gudang',0)}$, Offset {len(offset_hist)}")
        return
    ...
    print(f"🧠 Prob {final_prob*100:.0f}% {trend} Conf {conf*100:.0f}% ATR {atr:.2f} | MA {breakdown[0]*100:.0f}% MACD {breakdown[1]*100:.0f}% OFI {breakdown[2]*100:.0f}%")
    ...
    lot=0.05 if jenis=="PANEN KECIL" else 0.10
